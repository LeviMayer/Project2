...
def save_visualization(
    ...
    # Row 2: point heatmap (aggregated over slots)
    ax[1, 0].imshow(image_np)
    ax[1, 0].set_title("image")
    ax[1, 0].axis("off")

    ax[1, 1].imshow(gt_point_np, cmap="hot")
    ax[1, 1].set_title("gt_point_heatmap_agg")
    ax[1, 1].axis("off")

    ax[1, 2].imshow(pred_point_np, cmap="hot")
    ax[1, 2].set_title("pred_point_heatmap_agg")
    ax[1, 2].axis("off")
...
def main():
    ...
    dataset = LineHeatmapDataset(
        ...
        point_sigma=cfg["data"].get("point_sigma", 0.6),
        max_points=cfg["data"].get("max_points", 16),
    ...
    with torch.no_grad():
        for i in range(args.start_index, end_idx):
            ...
            gt_line_heatmap = sample["line_heatmap"]              # [1,H,W]
            gt_point_heatmaps = sample["point_heatmaps"]          # [K,H,W]
            point_valid_mask = sample["point_valid_mask"]         # [K]
            sample_id = sample.get("id", f"sample_{i:04d}")

            out = model(image)
            line_logits = out["line_logits"]      # [1,1,H,W]
            point_logits = out["point_logits"]    # [1,K,H,W]

            pred_line_heatmap = torch.sigmoid(line_logits).cpu()      # [1,1,H,W]
            pred_point_slots = torch.sigmoid(point_logits).cpu()      # [1,K,H,W]

            # aggregate GT/pred point heatmaps across valid slots
            valid_mask_cpu = point_valid_mask.view(-1, 1, 1).cpu()    # [K,1,1]
            gt_point_heatmaps_masked = gt_point_heatmaps.cpu() * valid_mask_cpu
            gt_point_heatmap_agg = gt_point_heatmaps_masked.max(dim=0, keepdim=True)[0]  # [1,H,W]

            pred_point_heatmap_agg = pred_point_slots.max(dim=1, keepdim=True)[0]         # [1,1,H,W]
            ...
            save_visualization(
                sample["image"],
                gt_line_heatmap,
                pred_line_heatmap.squeeze(0),          # [1,H,W]
                gt_point_heatmap_agg,                  # [1,H,W]
                pred_point_heatmap_agg.squeeze(0),     # [1,H,W]
                save_path,
            )

            # Save raw predictions for evaluation
            line_np = pred_line_heatmap.squeeze(0).squeeze(0).numpy().astype(np.float32)           # [H,W]
            point_slots_np = pred_point_slots.squeeze(0).numpy().astype(np.float32)                 # [K,H,W]
            point_agg_np = pred_point_heatmap_agg.squeeze(0).squeeze(0).numpy().astype(np.float32) # [H,W]
            valid_mask_np = point_valid_mask.cpu().numpy().astype(np.float32)                       # [K]

            np.save(out_dir / f"{sample_id}_line.npy", line_np)
            np.save(out_dir / f"{sample_id}_point_slots.npy", point_slots_np)
            np.save(out_dir / f"{sample_id}_point_agg.npy", point_agg_np)
            np.save(out_dir / f"{sample_id}_point_valid_mask.npy", valid_mask_np)

            print("saved:", save_path)
            print("saved:", out_dir / f"{sample_id}_line.npy")
            print("saved:", out_dir / f"{sample_id}_point_slots.npy")
            print("saved:", out_dir / f"{sample_id}_point_agg.npy")
            print("saved:", out_dir / f"{sample_id}_point_valid_mask.npy")
