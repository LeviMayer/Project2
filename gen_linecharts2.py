...
def choose_chart_style():
    """
    Returns dict with style params.
    """
...
def render_chart(
    ...
    # limits
    y_pad = (cfg.y_max - cfg.y_min) * 0.03
    ...
    if random.random() < cfg.axis_label_prob:
        ...
        # subtitle-like y-unit text above plot
        ax.text(
            0.0, 1.03,
...
def generate(cfg: GenConfig):
    ...
    with open(manifest, "w") as mf:
        for idx in range(cfg.n_samples):
            ...
            # write CSV labels (clean, no label noise)
